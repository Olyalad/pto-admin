...
class UserSearch extends User
{
    ...
    {
        return [
            [['id', 'sex', 'country_id', 'city_id', 'status', 'id_from', 'id_till', 'size',], 'integer'],    // Целочисленные значения
            [[
                'login', 'first_name', 'last_name', 'auth_key', 'password_hash', 'password_reset_token',
                'email_confirm_token', 'email', 'image', 'birthday', 'phone', 'address',
                'created_at', 'updated_at', 'created_at_from', 'created_at_till',
                'birthday_from', 'birthday_till', 'login_at', 'ip', 'login_at_from', 'login_at_till', 'groups'], 'safe'], // Безопасные аттрибуты
        ];
    }
    ...
    public function search($params)
    {
        ...
        if (Yii::$app->user->can('group_expert') && !Yii::$app->user->can('admin')) {
            $this->groups = Yii::$app->user->identity->groups;
            $query->andWhere(['groups' => Yii::$app->user->identity->groups]);
        }
		...
		if (Yii::$app->user->can('admin')) {
			$query->andFilterWhere(['groups' => $this->groups,]);
		}
